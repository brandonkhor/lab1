# User Page

## About Me
My name is **Brandon Khor** and I am a second-year Computer Science major at UCSD.
> I'm also in Warren College!

## Dislikes
As a programmer, I heavily dislike the following git command:

```
git push
```

This is because it sometimes gives me annoying errors.

## More Info

In my free time, I like going to this niche website called [YouTube](https://www.youtube.com/) and watch videos!

[If you're curious about my favorite programming language, click this!](/test.md)

Here is a list of my favorite artists:
1. Playboi Carti
2. Lil Uzi Vert
3. Beethoven

![Here is Beethoven!](https://hips.hearstapps.com/hmg-prod/images/beethoven-600x600.jpg?crop=1.00xw:1.00xh;0,0&resize=1200:*)
\
Some of my favorite songs in no particular order is:
- Timeless
- ILoveUIHateU
- 20 Min

Now here's a to-do list for my day:
- [x] Eat food
- [x] Finish this lab assignment
- [ ] Play badminton
# Links to the example headings above

Link to the About Me section: [Link Text](#about-me).

Link to the Dislikes section: [Link Text](#dislikes).

Link to the More Info section: [Link Text](#more-info).
